...
static core_os_timer* win32_timerCreate();
static void win32_timerDestroy(core_os_timer* timer);
static bool win32_timerSleep(core_os_timer* timer, int64_t duration_us);
static int64_t win32_timeNow(void);
static int64_t win32_timeDiff(int64_t end, int64_t start);
static int64_t win32_timeSince(int64_t start);
static int64_t win32_timeLapTime(int64_t* timer);
static double win32_timeConvertTo(int64_t delta, core_os_time_units units);
static uint64_t win32_timestampNow(void);
static uint64_t win32_timestampNow_precise(void);
static uint32_t win32_timestampToString(uint64_t ts, char* buffer, uint32_t max);
static core_os_file* win32_fileOpenRead(core_file_base_dir baseDir, const char* relPath);
static core_os_file* win32_fileOpenWrite(core_file_base_dir baseDir, const char* relPath);
static void win32_fileClose(core_os_file* f);
static uint32_t win32_fileRead(core_os_file* f, void* buffer, uint32_t len);
static uint32_t win32_fileWrite(core_os_file* f, const void* buffer, uint32_t len);
static uint64_t win32_fileGetSize(core_os_file* f);
static void win32_fileSeek(core_os_file* f, int64_t offset, core_file_seek_origin origin);
static uint64_t win32_fileTell(core_os_file* f);
static int32_t win32_fsSetBaseDir(core_file_base_dir whichDir, core_file_base_dir baseDir, const char* relPath);
static int32_t win32_fsGetBaseDir(core_file_base_dir whichDir, char* absPath, uint32_t max);
static int32_t win32_fsRemoveFile(core_file_base_dir baseDir, const char* relPath);
static int32_t win32_fsCopyFile(core_file_base_dir srcBaseDir, const char* srcRelPath, core_file_base_dir dstBaseDir, const char* dstRelPath);
static int32_t win32_fsMoveFile(core_file_base_dir srcBaseDir, const char* srcRelPath, core_file_base_dir dstBaseDir, const char* dstRelPath);
static int32_t win32_fsCreateDirectory(core_file_base_dir baseDir, const char* relPath);

core_os_api* os_api = &(core_os_api){
	.timerCreate = win32_timerCreate,
	.timerDestroy = win32_timerDestroy,
	.timerSleep = win32_timerSleep,
	.timeNow = win32_timeNow,
	.timeDiff = win32_timeDiff,
	.timeSince = win32_timeSince,
	.timeLapTime = win32_timeLapTime,
	.timeConvertTo = win32_timeConvertTo,
	.timestampNow = win32_timestampNow,
	.timestampToString = win32_timestampToString,
	.fileOpenRead = win32_fileOpenRead,
	.fileOpenWrite = win32_fileOpenWrite,
	.fileClose = win32_fileClose,
	.fileRead = win32_fileRead,
	.fileWrite = win32_fileWrite,
	.fileGetSize = win32_fileGetSize,
	.fileSeek = win32_fileSeek,
	.fileTell = win32_fileTell,
	.fsSetBaseDir = win32_fsSetBaseDir,
	.fsGetBaseDir = win32_fsGetBaseDir,
	.fsRemoveFile = win32_fsRemoveFile,
	.fsCopyFile = win32_fsCopyFile,
	.fsMoveFile = win32_fsMoveFile,
	.fsCreateDirectory = win32_fsCreateDirectory,
};
...
static bool win32_getInstallFolder(char* path_utf8, uint32_t max);
static bool win32_getTempFolder(char* path_utf8, uint32_t max);
static bool win32_getUserDataFolder(char* path_utf8, uint32_t max);
static bool win32_getUserAppDataFolder(char* path_utf8, uint32_t max);
static const char* win32_getBaseDirPathUTF8(core_file_base_dir baseDir);
static void win32_initKeycodes(void);
static void win32_initCursors(void);
...
bool core_os_initAPI(void)
{
	...
	if (!win32_getInstallFolder(s_OSContext.m_InstallDir, CORE_COUNTOF(s_OSContext.m_InstallDir))) {
		return false;
	}

	if (!win32_getTempFolder(s_OSContext.m_TempDir, CORE_COUNTOF(s_OSContext.m_TempDir))) {
		return false;
	}

	if (!win32_getUserDataFolder(s_OSContext.m_UserDataDir, CORE_COUNTOF(s_OSContext.m_UserDataDir))) {
		return false;
	}

	if (!win32_getUserAppDataFolder(s_OSContext.m_UserAppDataDir, CORE_COUNTOF(s_OSContext.m_UserAppDataDir))) {
		return false;
	}
...

	{
		...
		if (hasPreciseTimestamps) {
			os_api->timestampNow = win32_timestampNow_precise;
		}
	}
...
static int64_t win32_timeSince(int64_t start)
{
	return win32_timeNow() - start;
}

static int64_t win32_timeLapTime(int64_t* timer)
{
	const int64_t now = win32_timeNow();
	const int64_t dt = *timer == 0
		? 0
		: win32_timeDiff(now, *timer)
		;
	*timer = now;
	return dt;
}
...
static double win32_timeConvertTo(int64_t delta, core_os_time_units units)
{
	static const double kTimeConversionFactor[] = {
		1.0,          // CORE_TIME_UNITS_SEC
		1000.0,       // CORE_TIME_UNITS_MS
		1000000.0,    // CORE_TIME_UNITS_US
		1000000000.0, // CORE_TIME_UNITS_NS
	};
...
// File
//
#define FILE_FLAGS_ACCESS_Pos   0
#define FILE_FLAGS_ACCESS_Msk   (0x03u << FILE_FLAGS_ACCESS_Pos)
#define FILE_FLAGS_ACCESS_READ  ((0x01u << FILE_FLAGS_ACCESS_Pos) & FILE_FLAGS_ACCESS_Msk)
#define FILE_FLAGS_ACCESS_WRITE ((0x02u << FILE_FLAGS_ACCESS_Pos) & FILE_FLAGS_ACCESS_Msk)
#define FILE_FLAGS_BINARY_Pos   2
#define FILE_FLAGS_BINARY_Msk   (0x01u << FILE_FLAGS_BINARY_Pos)
...
static core_os_file* win32_fileOpenRead(core_file_base_dir baseDir, const char* relPath)
{
	const char* baseDirPath = win32_getBaseDirPathUTF8(baseDir);
	if (!baseDirPath) {
		return NULL;
	}
	...
	file->m_Flags = FILE_FLAGS_ACCESS_READ | FILE_FLAGS_BINARY_Msk;
...
static core_os_file* win32_fileOpenWrite(core_file_base_dir baseDir, const char* relPath)
{
	...
	core_os_file* file = (core_os_file*)CORE_ALLOC(s_OSContext.m_Allocator, sizeof(core_os_file));
	file->m_Handle = handle;
	file->m_Flags = FILE_FLAGS_ACCESS_WRITE | FILE_FLAGS_BINARY_Msk;
...
static int32_t win32_fsSetBaseDir(core_file_base_dir whichDir, core_file_base_dir baseDir, const char* relPath)
{
	...
	case CORE_FILE_BASE_DIR_INSTALL:
		return CORE_ERROR_INVALID_ARGUMENT;
	case CORE_FILE_BASE_DIR_USERDATA:
		win32_fsCreateDirectory(CORE_FILE_BASE_DIR_ABSOLUTE_PATH, absPath);
...
static int32_t win32_fsGetBaseDir(core_file_base_dir whichDir, char* absPath, uint32_t max)
{
	const char* baseDirPath = win32_getBaseDirPathUTF8(whichDir);
	if (!baseDirPath) {
		return CORE_ERROR_INVALID_ARGUMENT;
	}
...
static int32_t win32_fsCopyFile(core_file_base_dir srcBaseDir, const char* srcRelPath, core_file_base_dir dstBaseDir, const char* dstRelPath)
{
	...
	{
		const char* baseDirPath = win32_getBaseDirPathUTF8(srcBaseDir);
		if (!baseDirPath) {
			return CORE_ERROR_INVALID_ARGUMENT;
		}
	...
	{
		const char* baseDirPath = win32_getBaseDirPathUTF8(dstBaseDir);
		if (!baseDirPath) {
			return CORE_ERROR_INVALID_ARGUMENT;
		}
...
static bool win32_createDirectory_internal(const char* path)
{
	if (!path) {
		return false;
	}
...
static int32_t win32_fsCreateDirectory(core_file_base_dir baseDir, const char* relPath)
{
	...
	while (slash) {
		...
		if (!win32_createDirectory_internal(partialPath)) {
	...
	return win32_createDirectory_internal(absPath);
}
