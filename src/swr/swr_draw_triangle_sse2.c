...
void swrDrawTriangleSSE2(swr_context* ctx, int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t color0, uint32_t color1, uint32_t color2)
{
	...
	for (int32_t blockMinY = bboxMinY_aligned; blockMinY < bboxMaxY_aligned; blockMinY += 4) {
		...
		for (int32_t blockMinX = bboxMinX_aligned; blockMinX < bboxMaxX_aligned; blockMinX += 16) {
...
#if 0
			static const uint32_t kFirstSetBit[] = {
				0, // 0 = 0000b
				0, // 1 = 0001b
				1, // 2 = 0010b
				0, // 3 = 0011b
			...
			const uint32_t firstBlockID = kFirstSetBit[trivialRejectBlockMask];
			trivialRejectBlockMask >>= firstBlockID;
#else
			const uint32_t firstBlockID = 0;
#endif
			for (uint32_t iBlock = firstBlockID; trivialRejectBlockMask != 0;
